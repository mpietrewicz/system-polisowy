...
public interface ContractFinder extends JpaRepository<Contract, BaseAggregateRoot> {
    ...
    @Query(value = "SELECT SUM(ph.monthly_amount) " +
            ...
            "   AND ph1.since <= c.accountingMonth" +
            "   ORDER BY ph1.since DESC " +
            "   LIMIT 1 " +
            ")", nativeQuery = true)
    BigDecimal findContractPremium(@Param("contractId") String contractId);
    ...
    @Query(value = "SELECT ph.monthly_amount " +
            ...
            "AND cp.aggregateId.aggregateId = :componentId " +
            "AND cp.contract_id = c.aggregate_id " +
            "AND ph.since <= c.accountingMonth " +
            "ORDER BY ph.since DESC " +
            "LIMIT 1", nativeQuery = true)
    BigDecimal findComponentPremium(@Param("componentId") String componentId);
